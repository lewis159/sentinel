...
// CPU% from Prometheus (via the Clerk-gated /api/metrics/cpu, which runs the
// PromQL server-side). Shows the current cluster CPU% as a big value plus a
// small area sparkline of the last ~30 minutes. Themed with CSS vars; renders an
// "unreachable" state when Prometheus is down.

type Point = { ts: number; value: number };

// Build an SVG area path from points scaled into a viewBox. y is inverted
// (0% at the bottom). Width/height are viewBox units; CSS scales it to fit.
...
export function CpuWidget() {
  ...
  useEffect(() => {
    let cancelled = false;
    const load = () => {
      fetch('/api/metrics/cpu', { cache: 'no-store' })
        .then((r) => r.json())
        .then((d) => {
          if (cancelled) return;
  ...
  if (state === 'loading') return <div className="mon-state">Loading CPU…</div>;
  ...
  return (
    <div className="mon-cpu">
      <div className="mon-metric">
        ...
        <div className="mon-metric-l">cluster CPU</div>
      </div>
      {paths ? (
